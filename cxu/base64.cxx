...
string Base64::encode(const string& data)
{
    ...
    for (i = 0; i < len; ++i)
    {
        char c;
        c = (data[i] >> 2) & 0x3f;
        ret.append(1, base64Table[static_cast<int> (c)]);
        c = (data[i] << 4) & 0x3f;
        if (++i < len)
            c |= (data[i] >> 4) & 0x0f;
...
std::string Base64::encode(const ConstSegment & a)
{
    const size_t z = a.size();
    std::string s;
    s.resize(z);
    const char * p = a.ptr();
    for (auto & it : s) it = *p++;
...
void Base64::decode(const std::string & s, Buffer & a)
{
    const string ds = decode(s);
    ConstStringData csd(ds);
    a.copyin(ConstSegment(csd));
}


/*
size_t Base64EncodeFilter::request(const Segment & q)
{
        const size_t w = min(q.size() * 2 / 3, _s.size() - _p);
        std::string e = Base64::encode(ConstSegment(_s.ptr()+_p,w));
        q << ConstSegment(e.data(),e.length());
        _p += w;
        return e.length();
}
 */
